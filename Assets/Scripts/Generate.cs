...
public class Generate : MonoBehaviour
{
    ...
    [SerializeField]
    private bool sphere = true;

    private static Terrain terrén;
    ...
    private void Start()
    {
        terrén = terrain.GetComponent<Terrain>();
    }
    ...
    private void Generation()
    {
        ...
        for (int x = startPointX; x < chunkSize; x++)
        {
            for (int y = startPointY; y < chunkSize; y++)
            {
                for (int z = startPointZ; z < chunkSize; z++)
                {
    ...
    private void Update()
    {
        if (Input.GetKeyDown(KeyCode.G))
        {
            ...
            terrén.terrainData.SetHeights(0,0,heightmap);

            //offsets[0] = UnityEngine.Random.Range(0f, 1000f); offsets[1] = UnityEngine.Random.Range(0f, 1000f); offsets[2] = UnityEngine.Random.Range(0f, 1000f);
